<x-app-layout>
    ...
    <x-container class="py-8">
        <header class="mb-6">
            <h2 class="text-xl font-bold text-gray-900 tracking-tight">{{ __('app.dashboard.new_batch') }}</h2>
            <p class="mt-1 text-sm text-gray-500">{{ __('app.dashboard.new_batch_subtitle') }}</p>
        </header>

        <div class="bg-white border border-gray-200 rounded-xl shadow-sm divide-y divide-gray-100 overflow-hidden">
            <!-- Data & Model -->
            <div class="p-6 sm:p-8">
                <h3 class="text-sm font-semibold text-gray-900">{{ __('app.dashboard.data_model_heading') }}</h3>

                <div class="mt-4 max-w-sm">
                    <x-input-label for="model" :value="__('app.dashboard.model')" />
                ...
                <div class="mt-6">
                    <x-input-label :value="__('app.dashboard.dataset')" />
                    ...
                    >
                        ...
                        <p class="text-sm font-medium text-gray-700">
                            {{ __('app.dashboard.dropzone_text') }} <span class="text-navy-700 underline">{{ __('app.dashboard.browse') }}</span>
                        </p>
                        <p class="text-xs text-gray-400">{{ __('app.dashboard.dropzone_hint') }}</p>
                    </div>

                    <div class="mt-4 max-w-sm flex items-center gap-4">
                        <div class="h-px flex-1 bg-gray-200"></div>
                        <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">{{ __('app.dashboard.or') }}</span>
                        <div class="h-px flex-1 bg-gray-200"></div>
                    </div>

                    <x-text-input type="url" class="mt-4 block w-full max-w-sm" placeholder="{{ __('app.dashboard.google_sheets_placeholder') }}" />
                </div>
            </div>

            <!-- Prompt -->
            <div class="p-6 sm:p-8">
                <h3 class="text-sm font-semibold text-gray-900">{{ __('app.dashboard.prompt') }}</h3>
            ...
            <div class="px-6 sm:px-8 py-5 flex items-center justify-between bg-gray-50/60">
                <p class="text-xs text-gray-500">{{ __('app.dashboard.uses_api_key') }}</p>

                <x-primary-button type="button" class="gap-2">
                    {{ __('app.dashboard.run_batch') }}
