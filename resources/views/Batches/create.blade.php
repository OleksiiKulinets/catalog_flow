<x-app-layout>
    <x-slot name="header">
        <x-page-heading>{{ __('app.batches.create.title') }}</x-page-heading>
    </x-slot>

    <x-container class="py-8">
        <p class="text-sm text-gray-500 mb-6">{{ __('app.batches.create.subtitle') }}</p>

        <div class="bg-white border border-gray-200 rounded-xl shadow-sm divide-y divide-gray-100 overflow-hidden">
            <!-- Data & Model -->
            <div class="p-6 sm:p-8">
                <h3 class="text-sm font-semibold text-gray-900">{{ __('app.batches.create.data_model_heading') }}</h3>

                <div class="mt-4 max-w-sm">
                    <x-input-label for="model" :value="__('app.batches.create.model')" />
                ...
                <div class="mt-6">
                    <x-input-label :value="__('app.batches.create.dataset')" />
                    ...
                    >
                        ...
                        <p class="text-sm font-medium text-gray-700">
                            {{ __('app.batches.create.dropzone_text') }} <span class="text-navy-700 underline">{{ __('app.batches.create.browse') }}</span>
                        </p>
                        <p class="text-xs text-gray-400">{{ __('app.batches.create.dropzone_hint') }}</p>
                    </div>

                    <div class="mt-4 max-w-sm flex items-center gap-4">
                        <div class="h-px flex-1 bg-gray-200"></div>
                        <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">{{ __('app.batches.create.or') }}</span>
                        <div class="h-px flex-1 bg-gray-200"></div>
                    </div>

                    <x-text-input type="url" class="mt-4 block w-full max-w-sm" placeholder="{{ __('app.batches.create.google_sheets_placeholder') }}" />
                </div>
            </div>

            <!-- Prompt -->
            <div class="p-6 sm:p-8">
                <h3 class="text-sm font-semibold text-gray-900">{{ __('app.batches.create.prompt') }}</h3>
                ...
                <textarea
                    ...
                    placeholder="{{ __('app.batches.create.prompt_placeholder') }}"
                ></textarea>
            </div>

            <!-- Send -->
            <div class="px-6 sm:px-8 py-5 flex items-center justify-between bg-gray-50/60">
                <p class="text-xs text-gray-500">{{ __('app.batches.create.uses_api_key') }}</p>

                <x-primary-button type="button" class="gap-2">
                    {{ __('app.batches.create.run_batch') }}
