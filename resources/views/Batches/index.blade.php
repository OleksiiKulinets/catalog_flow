<x-app-layout>
    <x-slot name="header">
        <x-page-heading>{{ __('app.batches.title') }}</x-page-heading>
    </x-slot>
    ...
    <x-container class="py-8">
        <p class="text-sm text-gray-500 mb-6">{{ __('app.batches.subtitle') }}</p>
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <form method="GET" action="{{ route('batches.index') }}" class="flex flex-wrap items-center gap-3 p-4 border-b border-gray-100 bg-gray-50/60">
                <div class="relative flex-1 min-w-[220px]">
                    ...
                    <x-text-input
                        ...
                        placeholder="{{ __('app.batches.search_placeholder') }}"
                ...
                <select name="status" onchange="this.form.submit()" class="rounded-lg border-gray-300 bg-white text-sm focus:border-navy-500 focus:ring-navy-500">
                    <option value="">{{ __('app.batches.all_statuses') }}</option>
                ...
                <select name="model" onchange="this.form.submit()" class="rounded-lg border-gray-300 bg-white text-sm focus:border-navy-500 focus:ring-navy-500">
                    <option value="">{{ __('app.batches.all_models') }}</option>
                ...
                <select name="date" onchange="this.form.submit()" class="rounded-lg border-gray-300 bg-white text-sm focus:border-navy-500 focus:ring-navy-500">
                    <option value="">{{ __('app.batches.all_dates') }}</option>
                    <option value="today" @selected(($filters['date'] ?? '') === 'today')>{{ __('app.batches.date_today') }}</option>
                    <option value="week" @selected(($filters['date'] ?? '') === 'week')>{{ __('app.batches.date_week') }}</option>
                    <option value="month" @selected(($filters['date'] ?? '') === 'month')>{{ __('app.batches.date_month') }}</option>
                </select>
                ...
                <select name="sort" onchange="this.form.submit()" class="rounded-lg border-gray-300 bg-white text-sm focus:border-navy-500 focus:ring-navy-500">
                    <option value="newest" @selected(($filters['sort'] ?? 'newest') === 'newest')>{{ __('app.batches.sort_newest') }}</option>
                    <option value="oldest" @selected(($filters['sort'] ?? '') === 'oldest')>{{ __('app.batches.sort_oldest') }}</option>
                </select>
            </form>
            ...
            <div class="hidden sm:flex items-center gap-4 px-5 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wide border-b border-gray-100">
                <div class="flex-1 min-w-0">{{ __('app.batches.column_name') }}</div>
                <div class="shrink-0 w-32">{{ __('app.batches.column_status') }}</div>
                <div class="shrink-0 w-28">{{ __('app.batches.column_model') }}</div>
                <div class="shrink-0 w-36">{{ __('app.batches.column_progress') }}</div>
                <div class="shrink-0 w-24 text-right">{{ __('app.batches.column_created') }}</div>
            ...
            @if ($batches->total() > 0)
                <div class="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/60">
                    <p class="text-xs text-gray-500">
                        {{ __('app.batches.showing', ['from' => $batches->firstItem(), 'to' => $batches->lastItem(), 'total' => $batches->total()]) }}
                    </p>
