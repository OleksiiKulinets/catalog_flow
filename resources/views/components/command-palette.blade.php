@php
    $paletteItems = [
        ['key' => '1', 'label' => __('app.nav.dashboard'), 'url' => route('dashboard'), 'active' => request()->routeIs('dashboard')],
        ['key' => '2', 'label' => __('app.nav.batches'), 'url' => route('batches.index'), 'active' => request()->routeIs('batches.*')],
        ['key' => '3', 'label' => __('app.nav.settings'), 'url' => route('profile.edit'), 'active' => request()->routeIs('profile.edit')],
    ];
@endphp
...
>
    ...
    >
        <div class="px-4 py-3 border-b border-gray-100">
            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">{{ __('app.palette.title') }}</p>
        </div>
        ...
        <div class="px-4 py-2.5 border-t border-gray-100 bg-gray-50/60 flex items-center justify-between text-xs text-gray-400">
            <span>{{ __('app.palette.hint_select') }}</span>
            <span>{{ __('app.palette.hint_close') }}</span>
        </div>
    </div>
</div>
