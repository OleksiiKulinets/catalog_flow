        ...
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-3 min-w-0">
                <a
                    ...
                    aria-label="{{ __('batches.batch.back') }}"
                >
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 sm:p-8">
                    <h3 class="text-sm font-semibold text-gray-900">{{ __('batches.batch.prompt_used') }}</h3>
                    <p class="mt-4 text-sm text-gray-700 leading-relaxed">
                        {{ __('batches.batch.prompt_example') }}
                    </p>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 sm:p-8">
                    <div class="flex items-center justify-between">
                        <h3 class="text-sm font-semibold text-gray-900">{{ __('batches.batch.results') }}</h3>
                    ...
                    <div class="mt-4 flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-gray-200 bg-gray-50/60 px-6 py-10 text-center">
                        <p class="text-sm text-gray-500">{{ __('batches.batch.results_placeholder') }}</p>
                    </div>
                </div>
            </div>
            ...
            <!-- Overview -->
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 sm:p-8">
                <h3 class="text-sm font-semibold text-gray-900">{{ __('batches.batch.overview') }}</h3>

                <dl class="mt-4 space-y-4">
                    <div>
                        <dt class="text-xs text-gray-500">{{ __('batches.batch.model') }}</dt>
                        <dd class="mt-1 text-sm font-medium text-gray-900">{{ $batch['model'] }}</dd>
                    </div>

                    <div>
                        <dt class="text-xs text-gray-500">{{ __('batches.batch.dataset') }}</dt>
                        <dd class="mt-1 text-sm font-medium text-gray-900 truncate">{{ $batch['name'] }}</dd>
                    </div>

                    <div>
                        <dt class="text-xs text-gray-500">{{ __('batches.batch.created') }}</dt>
                        <dd class="mt-1 text-sm font-medium text-gray-900">{{ $batch['time'] }}</dd>
                    </div>

                    <div>
                        <dt class="text-xs text-gray-500">{{ __('batches.batch.progress') }}</dt>
                        <dd class="mt-2">
